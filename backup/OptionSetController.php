...
class OptionSetController extends Controller
{
    ...
    private function ensureHelperProductExists($shopModel)
    {
        ...
        try {
            ...
            // Check if existing product node matching title "Custom Option Fee" or "Custom Option Add-On" exists
            foreach ($products as $prod) {
                if (in_array(($prod['title'] ?? ''), ['Custom Option Fee', 'Custom Option Add-On'])) {
                    $firstVariant = $prod['variants']['nodes'][0] ?? null;
                    if ($firstVariant && !empty($firstVariant['id'])) {
                        $variantGid = $firstVariant['id'];
                        $productId = $prod['id'] ?? null;
                        $hasMedia = !empty($prod['media']['nodes']);

                        // Find first valid media that is not in FAILED state
                        $validMedia = null;
                        foreach ($prod['media']['nodes'] ?? [] as $m) {
                            if (($m['status'] ?? '') !== 'FAILED') {
                                $validMedia = $m;
                                break;
                            }
                        ...
                        $hasValidMedia = !empty($validMedia);
                        $variantHasImage = !empty($firstVariant['image']['id']);
                        $mediaId = $prod['media']['nodes'][0]['id'] ?? null;
                        $mediaId = $validMedia['id'] ?? null;

                        if (!$hasMedia && $productId) {
                            $this->attachHelperProductMedia($client, $productId);
                        if (!$hasValidMedia && $productId) {
                            // Helper product has no valid image -> auto-heal by uploading and linking badge
                            $this->attachHelperProductMedia($client, $productId, $variantGid);
                        } elseif (!$variantHasImage && $productId && $mediaId) {
                            // Product has valid media, but variant does not have an image -> link it
                            $this->linkVariantMedia($client, $productId, $variantGid, $mediaId);
                        }
            ...
            $badgeImageUrl = $this->getBadgeMediaSource($client);
            ...
            $variables = [
                'input' => [
                    'title' => 'Custom Option Fee',
                    'status' => 'ACTIVE',
                    'productType' => 'Custom Option Fee',
            ...
            $createdMediaId = $createBody['product']['media']['nodes'][0]['id'] ?? null;

            // set helper variant to product variant price
            ...
            if ($newVariantGid) {
                ...
                // Ensure media is attached and linked to variant
                if ($createdProductId && $createdMediaId) {
                    $this->linkVariantMedia($client, $createdProductId, $newVariantGid, $createdMediaId);
                } elseif ($createdProductId) {
                    $this->attachHelperProductMedia($client, $createdProductId);
                }
    ...
    {
        $badgePath = public_path('images/custom-option-badge.png');

        // 1. Direct upload to Shopify via Staged Uploads (no ngrok or external public URL required)
        if (file_exists($badgePath)) {
            try {
                $mimeType = \Illuminate\Support\Facades\File::mimeType($badgePath) ?: 'image/png';
                $filename = 'custom-option-badge.png';
                ...
                $stagedBody = $stagedRes->getDecodedBody();
                $target = $stagedBody['data']['stagedUploadsCreate']['stagedTargets'][0] ?? null;
                ...
                if ($target && !empty($target['url']) && !empty($target['resourceUrl'])) {
                    ...
                    $formData[] = [
                        'name' => 'file',
                        'contents' => fopen($badgePath, 'r'),
                        'filename' => $filename,
                    ];

                    $httpClient = new \GuzzleHttp\Client();
                    $uploadRes = $httpClient->post($target['url'], ['multipart' => $formData]);

                    if ($uploadRes->getStatusCode() >= 200 && $uploadRes->getStatusCode() < 300) {
                        \Log::info('Successfully uploaded badge image via Shopify staged upload: ' . $target['resourceUrl']);
                        return $target['resourceUrl'];
                    }
                } else {
                    \Log::warning('Staged upload target creation failed: ' . json_encode($stagedBody));
                }
            } catch (\Exception $e) {
                \Log::error('Failed to upload badge image via staged upload: ' . $e->getMessage());
            }
        }

        // 2. Dynamic URL Fallback (does not hardcode ngrok or any static tunnel)
        return $this->getDynamicBadgeImageUrl();
    }

    private function getDynamicBadgeImageUrl()
    {
        $host = env('SHOPIFY_APP_HOST_NAME');
        if (!empty($host)) {
            $scheme = (str_contains($host, 'localhost') || str_contains($host, '127.0.0.1')) ? 'http' : 'https';
            if (str_starts_with($host, 'http://') || str_starts_with($host, 'https://')) {
                return rtrim($host, '/') . '/images/custom-option-badge.png';
            }
            return "{$scheme}://{$host}/images/custom-option-badge.png";
        }

        $appUrl = env('SHOPIFY_APP_URL') ?: (config('app.url') ?: env('APP_URL'));
        if (!empty($appUrl)) {
            return rtrim($appUrl, '/') . '/images/custom-option-badge.png';
        }

        if (request() && request()->getHttpHost()) {
            return request()->getSchemeAndHttpHost() . '/images/custom-option-badge.png';
        }

        return asset('images/custom-option-badge.png');
    }
    ...
    private function linkVariantMedia($client, $productId, $variantId, $mediaId)
    {
        try {
            ...
            \Log::info("Linked media {$mediaId} to variant {$variantId} for product {$productId}");
        } catch (\Exception $e) {
            \Log::error("Failed to link variant media: " . $e->getMessage());
        }
    }

    private function attachHelperProductMedia($client, $productId)
    private function attachHelperProductMedia($client, $productId, $variantId = null)
    {
        try {
            $badgeImageUrl = $this->getBadgeMediaSource($client);
            if (empty($badgeImageUrl)) {
                \Log::warning("Could not obtain badge image URL for product {$productId}");
                return;
            }
            ...
            $mediaId = $res->getDecodedBody()['data']['productCreateMedia']['media'][0]['id'] ?? null;

            if ($mediaId) {
                // Fetch first variant ID of helper product
                $fetchQuery = <<<'QUERY'
                    ...
                    product(id: $id) {
                        variants(first: 1) {
                            nodes {
                                id
                if (!$variantId) {
                    // Fetch first variant ID of helper product if not provided
                    $fetchQuery = <<<'QUERY'
                    query getProductVariant($id: ID!) {
                        product(id: $id) {
                            variants(first: 1) {
                ...
                if ($variantId) {
                    $this->linkVariantMedia($client, $productId, $variantId, $mediaId);
                }
            }

            \Log::info("Attached badge image media and updated variant image for helper product {$productId}");
        } catch (\Exception $e) {
